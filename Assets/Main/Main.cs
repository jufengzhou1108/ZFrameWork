...
public class Main : MonoBehaviour
{
    ...
    private void RunResourceManagementTests()
    {
        RunCase("IResourceGroup_AddressablesGroup_ImplementsContract", () =>
        {
        ...
        {
            var manager = new AddressablesLoadManager();
            string managedKey = manager.BuildKey<int>("hero");
            Require(managedKey == "hero_Int32", "资源管理 key 应该追加泛型类型名称");
        });

        RunCase("AddressablesLoadManager_InvalidArguments_ReturnNullForSyncAndAsync", () =>
        {
            var manager = new AddressablesLoadManager();

            ZLog.Log("[TEST][EXPECTED-DIAGNOSTIC] 空 path 和空 key 会记录错误，但不应导致测试失败");
            Require(manager.Load<UnityEngine.Object>("", "missing") == null,
                "同步加载收到空 path 应该返回 null");
            Require(manager.Load<UnityEngine.Object>("missing", "") == null,
                "同步加载收到空 key 应该返回 null");
            Require(manager.LoadAsync<UnityEngine.Object>("", "missing")
                .GetAwaiter().GetResult() == null,
                "异步加载收到空 path 应该返回 null");
            Require(manager.LoadAsync<UnityEngine.Object>("missing", "")
                .GetAwaiter().GetResult() == null,
                "异步加载收到空 key 应该返回 null");
        });

        RunCase("AddressablesLoadManager_ReleaseAll_IsIdempotent", () =>
        {
            var manager = new AddressablesLoadManager();
            manager.ReleaseAll();
            manager.ReleaseAll();
            Require(manager.BuildKey<int>("after-release") == "after-release_Int32",
                "重复释放后的管理器仍应保持可用");
        });

        RunCase("AddressablesLoadManager_InvalidArgumentStress_DoesNotAbortSuite", () =>
        {
            const int iterations = 64;
            var manager = new AddressablesLoadManager();

            ZLog.Log($"[TEST][EXPECTED-DIAGNOSTIC] 重复执行 {iterations} 次空 key 校验会记录错误");
            for (int i = 0; i < iterations; i++)
            {
                Require(manager.Load<UnityEngine.Object>("missing", "") == null,
                    "无效 key 不应该创建资源条目或抛出异常");
            }
        ...
        RunCase("AddressablesGroup_ReleaseUnknownResource_IsNoOp", () =>
        {
